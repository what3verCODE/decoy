...
Make sessions first-class on the control client; collapse the dual client.
...
- `SessionRouter.sessionId` → **`SessionRouter.id`**; `HeaderSink`/`SessionRouter` are interface-only declarations now living alongside `Router`.
