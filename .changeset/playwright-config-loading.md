...
The only required argument is now the Playwright `target` (page/context) — the router loads the project's `decoy.config.*` (the same yaml/json sources the server reads, ADR-0007) and serves those definitions. `createRouterFixture()` takes no required arguments and discovers the config from `process.cwd()`.
