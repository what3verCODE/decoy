...
describe('examples/express — in-process middleware: serve mocks, fall through on miss', () => {
  let app: RunningApp
  ...
  test('an unmocked route falls through to the real host handler', async () => {
  ...
  test('a route neither mocked nor handled is the host app’s 404 (no real API to leak to)', async () => {
    ...
    // In-process the middleware falls through on a miss; with no downstream
    // handler the host app answers 404 (its own fail-closed). Nothing reaches a
    // real API, because there is none — that is the whole point of the mock.
