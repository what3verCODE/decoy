...
// server's `/admin` (the global session), e.g.
//   curl -X POST localhost:3004/admin/collection -d '{"name":"error-state"}'
...
startStack({ decoyPort: DECOY_PORT, spaPort: SPA_PORT, logger: createLogger() })
  .then((stack) => {
    ...
    console.log(`Decoy: ${stack.decoyBaseUrl}  (control: ${stack.decoyBaseUrl}/admin)`)
  })
  .catch((error: unknown) => {
