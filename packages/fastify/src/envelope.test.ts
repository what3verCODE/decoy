...
describe('toEnvelope', () => {
  test('splits path and query from the URL, leaving params to the engine', () => {
    const envelope = toEnvelope(req({ url: '/users/42?expand=true' }))
  ...
  test('repeated query keys become arrays; single keys stay scalar', () => {
    const envelope = toEnvelope(req({ url: '/search?tag=a&tag=b&q=x' }))
    expect(envelope.query).toEqual({ tag: ['a', 'b'], q: 'x' })
  })

  test('decodes a percent-encoded path', () => {
    const envelope = toEnvelope(req({ url: '/users/a%20b' }))
  ...
  test('takes the already-parsed body from request.body verbatim', () => {
    const body = { id: 7 }
    const envelope = toEnvelope(req({ method: 'POST', url: '/users', body }))
    expect(envelope.body).toBe(body)
  })

  test('body is undefined when Fastify left the request unparsed', () => {
    const envelope = toEnvelope(req({ method: 'POST', url: '/users' }))
