...
// Dogfood (ADR-0017): the panel's same-origin admin API is faked the way an adopter
...
// handle. Scoped to /admin/** so the SPA's own assets load from the preview server
...
export const test = base.extend<{ router: PlaywrightRouter }>({
  router: [createRouterFixture({ url: '**/admin/**' }), { auto: true }],
})
