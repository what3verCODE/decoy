...
test('opening a route shows its presets and variants', async ({ page }) => {
  ...
  // The catalog is shown first — no detail view yet.
  await expect(page.getByTestId('route-detail')).toHaveCount(0)
...
test('the back control returns to the routes catalog', async ({ page }) => {
  await page.goto('/')
  await page
    .getByTestId('route-row')
    .filter({ hasText: 'users-by-id' })
